...
// Fixture: 2 days, 3-hour intervals, offset = +1hr (3600s) so local hours
// are 1,4,7,10,13,16,19,22 — giving a single unambiguous midday (13:00) reading per day.
const OFFSET = 3600;
...
const day1 = [
  entry(1704067200, 10, "a0", "cond0"), // UTC 00:00 -> local 01:00
  entry(1704078000, 12, "a1", "cond1"), // UTC 03:00 -> local 04:00
  entry(1704088800, 15, "a2", "cond2"), // UTC 06:00 -> local 07:00
  entry(1704099600, 20, "a3", "cond3"), // UTC 09:00 -> local 10:00
  entry(1704110400, 25, "a4", "cond4"), // UTC 12:00 -> local 13:00 (midday match)
  entry(1704121200, 22, "a5", "cond5"), // UTC 15:00 -> local 16:00
  entry(1704132000, 18, "a6", "cond6"), // UTC 18:00 -> local 19:00
  entry(1704142800, 14, "a7", "cond7"), // UTC 21:00 -> local 22:00
];

const day2 = [
  entry(1704153600, 5, "b0", "cond0b"), // local 01:00
  entry(1704164400, 8, "b1", "cond1b"), // local 04:00
  entry(1704175200, 9, "b2", "cond2b"), // local 07:00
  entry(1704186000, 30, "b3", "cond3b"), // local 10:00
  entry(1704196800, 28, "b4", "cond4b"), // local 13:00 (midday match)
  entry(1704207600, 20, "b5", "cond5b"), // local 16:00
  entry(1704218400, 15, "b6", "cond6b"), // local 19:00
  entry(1704229200, 7, "b7", "cond7b"), // local 22:00
];
...
test("groupByDay + dailySummary together produce 5 or fewer day summaries from a real-shaped 40-entry dataset", () => {
  // Simulate OpenWeather's actual cadence: 40 entries, 3hr apart, starting at an arbitrary hour
  const start = 1704074400; // arbitrary, not day-aligned
  ...
  assert.ok(summaries.length <= 6); // 40 entries * 3hrs = 120hrs = 5 days, +partial boundary days
