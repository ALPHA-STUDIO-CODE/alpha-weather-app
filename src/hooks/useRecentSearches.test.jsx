...
describe('useRecentSearches', () => {
  ...
  it('recording 6 distinct cities keeps only the last 5, most-recent-first', () => {
    ...
    expect(result.current.recents.map((entry) => entry.name)).toEqual([
      'F',
      'E',
      'D',
      'C',
    ...
    expect(
      result.current.recents.some((entry) => entry.name === 'A'),
    ).toBe(false);
  });
  ...
  it('writes to storage on record, and a remount picks up the persisted list', () => {
    ...
    expect(JSON.parse(localStorage.getItem('awr_recent_searches'))).toEqual([
      city('Abuja', 'NG'),
    ]);
